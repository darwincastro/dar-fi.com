...
<Important content="You must generate in/out traffic using the Linux machine because you will capture packets throughout that interface, it won’t work capturing third Bluetooth devices not connected to the Linux machine." />

- Finish the packet capture with the shortcut: <Kbd hit={"Ctrl"} /> + <Kbd hit={"c"} />
